...
public sealed partial class WindowsAmbientContextService
{
    ...
    /// <summary>
    /// PrivacyClassification を構築する。
    /// Reason は MCP の get_policy 経由で LLM クライアントに渡る人間向けテキストなので、
    /// アプリ起動時点の <see cref="CultureInfo.CurrentUICulture"/> に応じて日本語/英語を選ぶ。
    /// 言語切替には再起動が必要 (起動時に 1 回だけ選択する設計)。
    /// </summary>
    ...
    {
        // 日本語環境のみ日本語、それ以外は全部英語へフォールバック (Strings.T と同じ方針)。
